...
#endif
#define FT7811_IRQ              ( OWL_EXT_IRQ_SIRQ0 )
#define FT7811_IRQ_GPIO        OWL_GPIO_PORTA(24)
int ft7811_sirq;
/*******************************************************************************
...
static void fts_report_value(struct fts_ts_data *data)
{
	...
	for (i = 0; i < event->touch_point; i++)
	{
		...
		{
			//rotate = 270 start
			//printk("event->au16_x[i]=%d----event->au16_y[i]=%d------------1---\n",event->au16_x[i],event->au16_y[i]);
        	        int tmp;
		            tmp = event->au16_x[i];
		            event->au16_x[i] = 768-event->au16_y[i];
		            event->au16_y[i] = tmp;

			//printk("event->au16_x[i]=%d----event->au16_y[i]=%d-----------2---\n",event->au16_x[i],event->au16_y[i]);		
			//rotate = 90 end
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	//printk("fts_parse_dt----------------1---\n");
	//ft7811_sirq = irq_of_parse_and_map(np, 0);
	...
	//printk("pdata->irq_gpio = %d------pdata->reset_gpio=%d------------------------1---",pdata->irq_gpio,pdata->reset_gpio);
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	// if(cfg_dts.rotate == 0 || cfg_dts.rotate == 180)
	//input_set_abs_params(input_dev, ABS_MT_POSITION_X, pdata->x_min, pdata->x_max, 0, 0);
	//input_set_abs_params(input_dev, ABS_MT_POSITION_Y, pdata->y_min, pdata->y_max, 0, 0);
	 //if(cfg_dts.rotate == 90 || cfg_dts.rotate == 270)
	input_set_abs_params(input_dev, ABS_MT_POSITION_X, pdata->x_min, pdata->y_max, 0, 0);
	input_set_abs_params(input_dev, ABS_MT_POSITION_Y, pdata->y_min, pdata->x_max, 0, 0);
	printk("pdata->x_min = %d------pdata->x_max=%d--------pdata->y_min = %d------pdata->y_max=%d-----------------1---\n",pdata->x_min, pdata->x_max,pdata->y_min, pdata->y_max);
...
*/
	...
	//printk("pdata->irqflags = %d------client->dev.driver->name=%s--------client->irq=%d----------------1---\n",pdata->irqflags,client->dev.driver->name,client->irq);

	//client->irq = FT7811_IRQ;
	//client->irq = gpio_to_irq(FT5X06_IRQ_GPIO);
	//client->irq = ft7811_sirq;
	//printk("pdata->irqflags = %d------client->dev.driver->name=%s--------client->irq=%d----------------1---\n",pdata->irqflags,client->dev.driver->name,client->irq);	
...
#endif
	//printk("client->irq=%d----------------2---\n",client->irq);
	enable_irq(client->irq);
	//printk("client->irq=%d----------------3---\n",client->irq);
	return 0;
