...
int ilitek_i2c_write(struct i2c_client *client, uint8_t * cmd, int length)
{
	...
	if(ret < 0)
	{
		printk(ILITEK_ERROR_LEVEL "%s, ilitek i2c write error, ret %d\n", __func__, ret);
	}
	return ret;
}
...
int ilitek_i2c_read(struct i2c_client *client, uint8_t *data, int length)
{
	...
	if(ret < 0){
		printk(ILITEK_ERROR_LEVEL "%s,ilitek i2c read error, ret %d\n", __func__, ret);
	}
		...
		int write_len, int delay, uint8_t *data, int read_len)
{
	...
	printk(ILITEK_ERROR_LEVEL "%s,ilitek i2c client->addr = %d\n", __func__, client->addr);
	#if 1
	if (read_len == 0) {
		if (write_len > 0) {
...
ilitek_i2c_esd_check(struct i2c_client *client, uint8_t *data, int length)
{
	...
	if (out_use == 0) {
		...
		if(ret < 0){
			...
			printk(ILITEK_ERROR_LEVEL "%s,ilitek i2c read error, ret %d,addr %x \n", __func__, ret,client->addr);
			return ret;
		}
	}
	else {
		data[0] = 0x02;
		printk("%s ilitek APK USE SO not check out_use = %d\n", __func__, out_use);
		return 0;
	}
	msleep(10);
	if (out_use == 0) {
...
static void ilitek_touch_esd_func(struct work_struct *work)
{
	...
	if(atomic_read(&ilitek_cmd_response) == 0){
		...
		printk("esd %s:ilitek have interrupt so not check!!!\n", __func__);
		goto ilitek_esd_check_out;
	}
	...
	for(i = 0; i < len; i++)
	{
		...
		if(ilitek_i2c_write_and_read(i2c.client, buf, 1, 0, buf, 7))
		{
			...
			switch ((mult_tp_id & 0xC0))
			{
				case TOUCH_POINT:
					{
						DBG("ilitek TOUCH_POINT  i = %d  (buf[0] & 0x3F) - 1 = %d\n", i, (buf[0] & 0x3F) - 1);
					...
					break;
				case RELEASE_POINT:
					DBG("ilitek RELEASE_POINT	i = %d	(buf[0] & 0x3F) - 1 = %d\n", i, (buf[0] & 0x3F) - 1);
					// release point
				#ifdef TOUCH_PROTOCOL_B
...
static int ilitek_report_data_2XX(void) {
			...
			switch ((mult_tp_id & 0xC0)){
				...
				case RELEASE_KEY:
					...
					for(key=0; key<sizeof(touch_key_code)/sizeof(touch_key_code[0]); key++){
						if(touch_key_press[key]){
							...
							DBG("ilitek Key:%d ID:%d release\n", touch_key_code[key], key);
							DBG(ILITEK_DEBUG_LEVEL "%s ilitek key release, %X, %d, %d\n", __func__, buf[0], x, y);
						}
				...
				DBG("TOUCH_KEY: key_id = %d\n", key_id);
					if((touch_key_press[key_id] == 0) && (touch_key_hold_press == 0 && i2c.keyflag)){
						...
						DBG(" ilitek Key:%d ID:%d press x=%d,touch_key_hold_press=%d,key_flag=%d\n", touch_key_code[key_id], key_id,x,touch_key_hold_press,i2c.keyflag);
					}
					break;
				#endif
				case TOUCH_POINT:
					...
					{
						// report to android system
						DBG(" ilitek Point, ID=%02X, X=%04d, Y=%04d,touch_key_hold_press=%d\n",buf[0]  & 0x3F, x,y,touch_key_hold_press);
						ilitek_touch_down((buf[0] & 0x3F)-1, x, y, 10);
						ret=0;
					}
					break;
					...
					if (touch_key_hold_press !=0 && i==0){
						for(key=0; key<sizeof(touch_key_code)/sizeof(touch_key_code[0]); key++){
							if(touch_key_press[key]){
								...
								touch_key_press[key] = 0;
								DBG(" ilitek Key:%d ID:%d release\n", touch_key_code[key], key);
								DBG(ILITEK_DEBUG_LEVEL "%s  ilitek key release, %X, %d, %d,touch_key_hold_press=%d\n", __func__, buf[0], x, y,touch_key_hold_press);
							}
							touch_key_hold_press=0;
							//ret = 1;// stop timer interrupt
						}
	...
	if(len == 0){
		...
		i2c.keyflag = 0;
		DBG(" ilitek Release3, ID=%02X, X=%04d, Y=%04d\n",buf[0]  & 0x3F, x,y);
		ret = 1;
	...
	for(i = 0; i < max_point; i++){
		...
		if(tp_status){
			...
			if(i2c.keyflag == 0){
				for(j = 0; j < i2c.keycount; j++){
					if((x >= i2c.keyinfo[j].x && x <= i2c.keyinfo[j].x + i2c.key_xlen) && (y >= i2c.keyinfo[j].y && y <= i2c.keyinfo[j].y + i2c.key_ylen)){
						...
						DBG(" ilitek Key, Keydown ID=%d, X=%d, Y=%d, key_status=%d,keyflag=%d\n", i2c.keyinfo[j].id ,x ,y , i2c.keyinfo[j].status,i2c.keyflag);
						break;
					}
				}
			}
			...
			if(touch_key_hold_press == 0){
				i2c.keyflag = 1;
				ilitek_touch_down(i, x, y, 10);
				DBG(" ilitek Point, ID=%02X, X=%04d, Y=%04d,release_flag[%d]=%d,tp_status=%d,keyflag=%d\n",i, x,y,i,i2c.release_flag[i],tp_status,i2c.keyflag);
			}
			#ifdef VIRTUAL_KEY_PAD
			if(touch_key_hold_press == 1){
				for(j = 0; j <= i2c.keycount; j++){
					if((i2c.keyinfo[j].status == 1) && (x < i2c.keyinfo[j].x || x > i2c.keyinfo[j].x + i2c.key_xlen || y < i2c.keyinfo[j].y || y > i2c.keyinfo[j].y + i2c.key_ylen)){
						...
						touch_key_hold_press = 0;
						DBG(" ilitek Key, Keyout ID=%d, X=%d, Y=%d, key_status=%d\n", i2c.keyinfo[j].id ,x ,y , i2c.keyinfo[j].status);
						break;
					}
				}
			}
			...
			//release_flag[i] = 0;
			i2c.release_flag[i] = 0;
			DBG(" ilitek Point, ID=%02X, X=%04d, Y=%04d,release_flag[%d]=%d,tp_status=%d\n",i, x,y,i,i2c.release_flag[i],tp_status);
			input_mt_sync(i2c.input_dev);
			#endif
		}
			...
			if (touch_key_hold_press == 1){
				for(i = 0; i < i2c.keycount; i++){
					if(i2c.keyinfo[i].status){
						...
						touch_key_hold_press = 0;
						DBG(" ilitek Key, Keyup ID=%d, X=%d, Y=%d, key_status=%d, touch_key_hold_press=%d\n", i2c.keyinfo[i].id ,x ,y , i2c.keyinfo[i].status, touch_key_hold_press);
					}
				}
			}
			#endif
		}
		DBG(" ilitek release_counter=%d,packet=%d\n",release_counter,packet);
	}
#ifdef TOUCH_PROTOCOL_B
	input_mt_report_pointer_emulation(i2c.input_dev, true);
#endif
...
static void ilitek_i2c_irq_work_queue_func(struct work_struct *work)
{
	...
	if (ret == 0){
		...
		msleep(100);
		DBG(ILITEK_ERROR_LEVEL "%s,  ilitek process error\n", __func__);
		ilitek_i2c_irq_enable();
	}
#endif
}
...
static irqreturn_t ilitek_i2c_isr(int irq, void *dev_id)
{
	...
	if (update_wait_flag == 1) {
		DBG(ILITEK_ERROR_LEVEL "%s 11111  ilitek update_wait_flag =1 return\n",__func__);
		return IRQ_HANDLED;
	}
	#endif
	#endif
	...
	if(update_Flag == 1){
		DBG(ILITEK_ERROR_LEVEL "%s 11111 ilitek  update_Flag =1\n",__func__);
		int_Flag = 1;
	}
	else{
...
static int ilitek_i2c_update_thread(void *arg)
{
	...
	printk("ilitek reset_request_success = %d,\n",i2c.reset_request_success);
	...
	if(ret){
		printk(ILITEK_ERROR_LEVEL "%s,  ilitek register input device, error\n", __func__);
		return ret;
	}
	printk(ILITEK_ERROR_LEVEL "%s,  ilitek register input device, success\n", __func__);


	printk(ILITEK_DEBUG_LEVEL "%s, ilitek  exit\n", __func__);
	return ret;
}
#endif
#endif
...
static int ilitek_i2c_touchevent_thread(void *arg)
{
	...
	printk(ILITEK_DEBUG_LEVEL "%s,  ilitek enter\n", __func__);
	...
	while(1){
		...
		if(kthread_should_stop()){
			printk(ILITEK_DEBUG_LEVEL "%s,  ilitek stop\n", __func__);
			break;
		}
		...
		if(ilitek_i2c_process_and_report() < 0){
			msleep(3000);
			printk(ILITEK_ERROR_LEVEL "%s, ilitek  process error\n", __func__);
		}
		#ifdef CLOCK_INTERRUPT
		ilitek_i2c_irq_enable();
		#endif
...
static int ilitek_i2c_early_suspend(struct early_suspend *h)
{
...
/*
   ...
   nothing
 */
static int ilitek_i2c_late_resume(struct early_suspend *h)
{
	printk("%s\n", __func__);
...
#ifdef CLOCK_INTERRUPT
			if(Request_IRQ()){
				printk(ILITEK_ERROR_LEVEL "%s, ilitek  request irq, error\n", __func__);
			}
			else{
				...
				i2c.irq_status = 1;
				printk(ILITEK_ERROR_LEVEL "%s, ilitek  request irq, success\n", __func__);
			}
#else
...
int ilitek_i2c_read_tp_info( void)
{
	...
	int res_len = 0;
		...
		for (i = 0; i < 20; i++) {
			...
			if (buf[1] >= 0x80) {
				printk(" ilitek FW is ready  ok ok \n");
				break;
			}else {
				msleep(50);
			}
		...
		printk(ILITEK_DEBUG_LEVEL "%s, ilitek  firmware version:%d.%d.%d\n", __func__, buf[0], buf[1], buf[2]);
		...
		printk(ILITEK_DEBUG_LEVEL "%s, ilitek  protocol version:%d.%d\n", __func__, buf[0], buf[1]);
		...
		printk(ILITEK_DEBUG_LEVEL "%s,  ilitek MCU KERNEL version:%d.%d.%d.%d.%d\n", __func__, buf[0], buf[1], buf[2],buf[3], buf[4]);
		...
		if (!serial23) {
			...
			printk(ILITEK_DEBUG_LEVEL "%s,  ilitek firmware version:%d.%d.%d.%d\n", __func__, buf[0], buf[1], buf[2],buf[3]);
			for(i = 0; i < 4; i++)
			{
			...
			printk(ILITEK_DEBUG_LEVEL "%s, ilitek  firmware version:%d.%d.%d.%d.%d.%d.%d.%d\n", __func__, buf[0], buf[1], buf[2],buf[3], buf[4], buf[5], buf[6],buf[7]);
			for(i = 0; i < 8; i++)
			{
				i2c.firmware_ver[i] = buf[i];
			}
		...
		printk(ILITEK_DEBUG_LEVEL "%s,  ilitek max_x: %d, max_y: %d, ch_x: %d, ch_y: %d\n",
		__func__, i2c.max_x, i2c.max_y, i2c.x_ch, i2c.y_ch);

		if((i2c.protocol_ver & 0xFF00) == 0x200){
			printk(ILITEK_DEBUG_LEVEL "%s,  ilitek max_tp: %d, max_btn: %d\n", __func__, i2c.max_tp, i2c.max_btn);
		}
		else if((i2c.protocol_ver & 0xFF00) == 0x300){
			printk(ILITEK_DEBUG_LEVEL "%s,  ilitek max_tp: %d, max_btn: %d, key_count: %d\n", __func__, i2c.max_tp, i2c.max_btn, i2c.keycount);
			...
			printk(ILITEK_DEBUG_LEVEL "%s,  ilitek key_xlen: %d, key_ylen: %d\n", __func__, i2c.key_xlen, i2c.key_ylen);
			...
			for(i = 0; i < i2c.keycount; i++){
				...
				i2c.keyinfo[i].status = 0;
				printk(ILITEK_DEBUG_LEVEL "%s,  ilitek key_id: %d, key_x: %d, key_y: %d, key_status: %d\n", __func__, i2c.keyinfo[i].id, i2c.keyinfo[i].x, i2c.keyinfo[i].y, i2c.keyinfo[i].status);
			}
		}
	#endif
	printk(ILITEK_DEBUG_LEVEL " ilitek %s, min_x: %d, max_x: %d, min_y: %d, max_y: %d, ch_x: %d, ch_y: %d, max_tp: %d, key_count: %d\n"
...
int ilitek_i2c_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if(ret < 0){
		printk(ILITEK_ERROR_LEVEL "%s,  ilitek 0x01 0x00 set tp suspend err, ret %d\n", __func__, ret);
	}
	msleep(30);
	cmd[0] = 0x0A;
	cmd[1] = 0x01;
	ret = ilitek_i2c_transfer(client, msgs_cmd, 1);
	if(ret < 0){
		printk(ILITEK_ERROR_LEVEL "%s, ilitek  0x0A 0x01 set tp suspend err, ret %d\n", __func__, ret);
	}
	...
	if(i2c.valid_irq_request != 0){
		...
		i2c.stop_polling = 1;
		printk(ILITEK_DEBUG_LEVEL "%s,  ilitek stop i2c thread polling\n", __func__);
	}
#if IC2120
	...
	ret = ilitek_i2c_transfer(client, msgs_cmd, 1);
	if(ret < 0){
		printk(ILITEK_ERROR_LEVEL "%s,  ilitek 0x02 0x00 set tp suspend err, ret %d\n", __func__, ret);
	}
#else
	msgs_cmd[0].len = 1;
	cmd[0] = ILITEK_TP_CMD_SLEEP;
...
int ilitek_i2c_resume(struct i2c_client *client)
{
	...
	if(i2c.valid_irq_request != 0){
		...
		i2c.stop_polling = 0;
		printk(ILITEK_DEBUG_LEVEL "%s,  ilitek start i2c thread polling\n", __func__);
	}
#ifdef HALL_CHECK
#if HALL_CHECK == HALL_CHECK_OTHER
	if(!curr_hall_state)
		...
		const struct i2c_device_id *id)
{
	...
	i2c.client = client;
	printk(ILITEK_DEBUG_LEVEL "%s, ilitek  i2c new style format\n", __func__);
	...
	printk(ILITEK_DEBUG_LEVEL "%s, ilitek  add i2c device, success\n", __func__);
	
	if(i2c.client == NULL){
		printk(ILITEK_ERROR_LEVEL "%s,  ilitek no i2c board information\n", __func__);
		return -1;
	} else {
	}
	if((i2c.client->addr == 0) || (i2c.client->adapter == 0)){
		printk(ILITEK_ERROR_LEVEL "%s, ilitek  invalid register\n", __func__);
		return ret;
	}
	...
	if(i2c.input_dev == NULL){
		printk(ILITEK_ERROR_LEVEL "%s,  ilitek allocate input device, error\n", __func__);
		return -1;
	}
#ifdef ILI_UPDATE_FW
	ret = 0;
	...
	if (ret == 0) {
		...
		if(thread_update == (struct task_struct*)ERR_PTR){
			thread_update = NULL;
			printk(ILITEK_ERROR_LEVEL "%s, ilitek thread_update kthread create, error\n", __func__);
		}
	}
	#endif
#else
...
static int ilitek_i2c_remove(struct i2c_client *client)
{
	...
	if(i2c.client->irq != 0){
		if(i2c.valid_irq_request != 0){
			...
			if(i2c.irq_work_queue){
				...
				printk(ILITEK_DEBUG_LEVEL "%s, ilitek  destory work queue\n", __func__);
			}
		}
#ifdef CLOCK_INTERRUPT
#ifdef REPORT_THREAD
		if(i2c.thread != NULL){
			...
			printk(ILITEK_DEBUG_LEVEL "%s,  ilitek stop i2c thread\n", __func__);
		}
#endif
#endif
	}
	...
	if(i2c.valid_input_register != 0){
		...
		printk(ILITEK_DEBUG_LEVEL "%s, ilitek  unregister i2c input device\n", __func__);
	}
...
static int ilitek_init(void)
{
	...
	if(ret != 0)
	{
		printk(ILITEK_ERROR_LEVEL "%s, ilitek  add i2c device, error\n", __func__);
		return ret;
	}
...
//#endif
	...
	if(i2c.client->irq != 0){
		if(i2c.valid_irq_request != 0){
			free_irq(i2c.client->irq, &i2c);
			printk(ILITEK_DEBUG_LEVEL "%s, ilitek  free irq\n", __func__);
			if(i2c.irq_work_queue){
				destroy_workqueue(i2c.irq_work_queue);
				printk(ILITEK_DEBUG_LEVEL "%s,  ilitek destory work queue\n", __func__);
			}
	...
	if(i2c.valid_i2c_register != 0){
		...
		printk(ILITEK_DEBUG_LEVEL "%s,  ilitek delete i2c driver\n", __func__);
	}
	if(i2c.valid_input_register != 0){
		input_unregister_device(i2c.input_dev);
		printk(ILITEK_DEBUG_LEVEL "%s,  ilitek unregister i2c input device\n", __func__);
